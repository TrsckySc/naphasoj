...
function alertInfo(content, type) {
  ...
  alertEle.className =
    "alert position-fixed w-50" + " alert-" + (type || "success");
  $("#alert").fadeIn("fast");
...
function handleSwaggerJSON(res, owner) {
  Object.keys(res.paths).forEach(function (pathKey) {
    var pathData = res.paths[pathKey];
    Object.keys(pathData).forEach(function (methodKey) {
      var methodData = pathData[methodKey];
      // 响应数据
      var resData = {};

      var responses = Object.keys(methodData.responses);
      var response = null;
  ...
  tagList = res.tags;
  var swaggerListTemplate = Handlebars.compile(
    $("#swagger-path-template").html()
  );
...
function getSwaggerJsonData(owner) {
  if (
    $("#swagger-json-address").val() === "" ||
    $("#swagger-json-address").val() === undefined
  ) {
  ...
  new SwaggerClient($("#swagger-json-address").val()).then(function (
    swaggerData
  ) {
    if (JSON.stringify(swaggerData.spec) === "{}") {
      alertInfo("该地址不是标准的JSON数据结构", "danger");
      $(owner).css("display", "");
      $(owner).next().css("display", "none");
      return;
    }
    handleSwaggerJSON(swaggerData.spec, owner);
  });
}

function handleChangeFile(owner) {
  console.log(owner.files);
}

function getSwaggerJsonLocalData(owner) {
  if (
    $("#swagger-json-local").val() === "" ||
    $("#swagger-json-local").val() === undefined
  ) {
    alertInfo("请选择swagger json文件", "danger");
    return;
  }
  $(owner).css("display", "none");
  $(owner).next().css("display", "");
  ...
  reader.readAsText($("#swagger-json-local").get(0).files[0]);
  reader.onload = (res) => {
    try {
...
function changeTagPaths(owner, e) {
  ...
  $(owner)
    .parent()
    .next()
    .find("input[name='swagger-tag-path']")
...
function confirmImport() {
  ...
  var param = []; // 保存的接口列表
  $("#swagger-path-warp")
    .find("input[name='swagger-tag-path']:checked")
    .each(function (index, data) {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.success) {
        ...
        $("#interface-prefix").text(
          data.data.prefix ? data.data.prefix : "无接口前缀"
        );
